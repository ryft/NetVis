...
\teaser{
\centering
\includegraphics[width=0.32\linewidth]{materials/distribution-tease2}
\includegraphics[width=0.3\linewidth]{materials/dataflow-tease2}
\includegraphics[width=0.3\linewidth]{materials/cube}

\centering
\includegraphics[width=0.3\linewidth]{materials/traffic-volume-tease2}
\includegraphics[width=0.3\linewidth]{materials/heat-map-tease2}
\includegraphics[width=0.3\linewidth]{materials/groups-tease2}
...
Computer network traffic visualizations deliver improved understanding of traffic on a network to an
observer. Existing tools often opt for graph or plot-based visualizations to detect patterns or
outliers in the data, but they still largely provide a segmented view of any data feed. In this
paper, we present a novel network traffic visualization framework that makes use of a variety of
complementary visualizations to obtain better situational awareness. Our proposed solution; NetVis
provides six different visualizations that work in tandem to provide different perspectives of the
same data in real-time. As each visualization is modularly tied together it enables a user to
investigate on-going activity or any subset of it at their pace and what they believe is more
important to explore further. Three of the visualizations are based on existing literature (parallel
coordinate plots, flowscan and spinning cube of potential doom), whereas the three remaining
visualizations are novel to this paper. Our results show that it is possible to use the tool to
detect unusual activity and cyber attacks on a network. The framework is written in a way that
allows future visualizations to be added straightforwardly.
...
Analysing and monitoring network traffic is becoming an important part of network security. As
the amounts of data to process increases substantially each year, it is becoming more and more
difficult to make effective analysis of on-going activity on a network. Visualizations offer a
very useful solution: they give a compact representation of data to a human analyst who can use them
to detect patterns in the network \cite{ware2012information}. An effective visualization of network
traffic makes it easy to identify outliers while making clear what general patterns are arising, and
will facilitate detection of malicious activity.

In this paper, we present a software application with a novel workflow paradigm in mind. It is
