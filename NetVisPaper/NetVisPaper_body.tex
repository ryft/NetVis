...
networks, and via such enhanced awareness can facilitate the deteciton of malicious traffic.
...
present a novel framework designed to support multiple heterogeneous vizualisations of network
...
analysts wtih vizualisations are typically configured around propritary Security Incident Event
Management (SIEM) environments, and so are by their nature limited in scope to a finite set of
vizualisations. In contrast, our view is that the visulization should be working in tandem to
...
developed a framework which can easily support new vizualisations as well as the tailoring of
existing ones.
