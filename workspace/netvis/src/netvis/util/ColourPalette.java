...
public class ColourPalette {
	...
	public Color getNextColour() {
		...
		Color thisColour;
		if (currentIndex < colours[SCHEME].length) {

			thisColour = colours[SCHEME][currentIndex];
			currentIndex++;

		} else {
			thisColour = colours[SCHEME][currentIndex-1];
		}
	...
	public void setColour(GL2 gl, Color colour) {
		...
		gl.glColor3d(normalise(colour.getRed()), normalise(colour.getGreen()), normalise(colour.getBlue()));
