...
public class CometPainter implements NodePainter {
	static {
	...
	public void DrawNode (int base, Node lum, GL2 gl)
	{
		...
		int x = 0; //lum.getCenter().x;
		int y = 0; //lum.getCenter().y;
		...
		if (lum.getSelected() == true)
			Painter.DrawImage (TexturePool.get("hexagon2"), x, y, 2*base/512.0, 0, gl);
		else
			Painter.DrawImage (TexturePool.get("hexagon1"), x, y, 2*base/512.0, 0, gl);
		...
		for (Comet i : lum.getEntities())
		{
			if (lum.getSelected())
			{
				DrawEntity (i, new Position(0, 0), gl);
				DrawTail   (i, new Position(0, 0), gl);
				//DrawTrace  (i, new Position(0, 0, gl);
			}
		}
	}
